...
export default function CreateAdminChannelModal({ isOpen, onClose }: CreateAdminChannelModalProps) {
  ...
  const [createAdminChannel, { loading }] = useMutation(CREATE_ADMIN_CHANNEL_MUTATION, {
    refetchQueries: [{ query: GET_ADMIN_CHANNELS_QUERY }],
    onCompleted: () => {
      toast.success('Chaîne de publication créée avec succès');
      handleClose();
    },
    onError: (error) => {
        ...
        <div className="bg-gradient-to-r from-purple-600 to-purple-700 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold">Nouvelle Chaîne de Publication</h2>
            </div>
            <button
              onClick={handleClose}
              disabled={loading}
          ...
          <p className="text-purple-100 text-sm mt-2">
            Ajoutez une chaîne YouTube pour publier des shorts
          </p>
        </div>
