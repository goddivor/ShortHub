...
import { UPDATE_SOURCE_CHANNEL_MUTATION, GET_SOURCE_CHANNELS_QUERY } from '@/lib/graphql';
import { ContentType, SourceChannel } from '@/types/graphql';
import { useToast } from '@/context/toast-context';
import SpinLoader from '@/components/SpinLoader';
import { CloseCircle, VideoPlay, Edit } from 'iconsax-react';

interface EditSourceChannelModalProps {
  isOpen: boolean;
  onClose: () => void;
  channel: SourceChannel | null;
}

export default function EditSourceChannelModal({ isOpen, onClose, channel }: EditSourceChannelModalProps) {
  ...
  const [updateSourceChannel, { loading }] = useMutation(UPDATE_SOURCE_CHANNEL_MUTATION, {
    refetchQueries: [{ query: GET_SOURCE_CHANNELS_QUERY }],
    onCompleted: () => {
      toast.success('Chaîne source modifiée avec succès');
      onClose();
    },
    onError: (error) => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 text-white">
          ...
          <p className="text-blue-100 text-sm mt-2">
            Modifiez les informations de la chaîne source
          </p>
        </div>
