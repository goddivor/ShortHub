// src/pages/assistant/AssistantCalendarPage.tsx
...
const AssistantCalendarPage: React.FC = () => {
  const { user } = useAuth();
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-purple-100 text-sm">Vidéaste: {user?.assignedTo?.username}</span>
            </div>
            <h1 className="text-3xl font-bold mb-2">Calendrier</h1>
            <p className="text-purple-100 text-lg">
              Visualisez les deadlines et le planning du vidéaste
            </p>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`
                      ...
                      ${isTodayDate ? 'bg-cyan-50 border-cyan-300' : ''}
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`
                          ...
                          ${!isCurrentMonth ? 'text-gray-400' : isTodayDate ? 'text-cyan-600' : 'text-gray-900'}
                        `}
                      >
                        {format(day, 'd')}
                      </span>
      ...
      {selectedDate && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            Vidéos pour le {format(selectedDate, 'PPP', { locale: fr })}
          </h3>

          {getShortsForDate(selectedDate).length === 0 ? (
            <p className="text-gray-500 text-center py-8">Aucune vidéo pour cette date</p>
          ) : (
